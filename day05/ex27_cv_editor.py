...
import cv2
import numpy as np
import time # 저장 시 시분초를 이미지 이름에 사용
...
def update(_=None):
    ...
    global edited_img   # 전역변수를 사용하겟다는 키워드
    ...
    threshold = cv2.getTrackbarPos('Threshold',winname) # 이진화 값 가져오기
    ...
    # 이진화
    if threshold > 0:
        gray = cv2.cvtColor(edited, cv2.COLOR_BGR2GRAY) # 그레이 스케일
